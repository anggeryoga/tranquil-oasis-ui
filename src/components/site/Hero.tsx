...
import cardHouse from "@/assets/card-house.jpg";
  ...
  return (
    <section className="relative overflow-hidden rounded-[20px]">
      <motion.img
        ...
        width={1920}
        height={1088}
        alt="Rustic cabins in an open meadow beneath a snow-capped mountain range at dusk"
        className="h-[420px] w-full object-cover sm:h-[520px] lg:h-[600px]"
        initial={{ scale: 1.06 }}
        animate={{ scale: 1 }}
        transition={{ duration: 2.4, ease: [0.22, 1, 0.36, 1] }}
      />
      <div className="absolute inset-0 bg-gradient-to-b from-foreground/15 via-transparent to-foreground/10" />

      <motion.h1
        initial={{ opacity: 0, y: 18 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.9, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
        className="display absolute top-[8%] left-1/2 w-full -translate-x-1/2 px-6 text-center text-[clamp(2rem,6.2vw,5.1rem)] text-card"
      >
        <span className="block">
          Serenity's <em className="font-normal italic">Retreat.</em>
        </span>
        <span className="block">
          <em className="font-normal italic">Your Private</em> Oasis
        </span>
      </motion.h1>
      ...
      <div className="absolute bottom-[38%] left-[12%] hidden gap-2 lg:flex">
        <CircleButton label="Previous" variant="white">
          <Arrow className="rotate-[225deg]" />
        </CircleButton>
        <CircleButton label="Next" variant="white">
      ...
      >
        <div className="hairline grid size-14 place-items-center rounded-full text-center leading-none">
          <span>
            <span className="block text-[15px] font-medium">126</span>
            <span className="block text-[8px] text-muted-foreground">/122</span>
          </span>
        </div>
        ...
        <img
          ...
          width={800}
          height={640}
          alt="Poolside patio at the retreat"
      ...
      >
        <div className="mb-2 flex items-center justify-between px-1">
          <span className="text-[10px] text-muted-foreground">panoramic view</span>
          <Arrow />
        </div>
        <div className="relative overflow-hidden rounded-[10px]">
          <img
            ...
            width={800}
            height={640}
            alt="Desert house with a vintage car"
            className="h-24 w-full object-cover"
          />
          <Pill as="div" variant="white" className="absolute inset-x-0 bottom-2 mx-auto h-6 w-fit px-2.5">
            <Play className="size-2.5" strokeWidth={1.5} /> play
          </Pill>
        </div>
        <div className="mt-2 flex justify-start">
          <CircleButton label="Expand video" variant="white" className="size-7">
            <Arrow />
          </CircleButton>
        </div>
      </motion.div>
